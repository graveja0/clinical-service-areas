...
```{r}
get_percent_within <- function(sf) {
    zips_within <- data.frame(zip_code = sf_zip_cent$zip_code, geo = st_within(sf_zip_cent,sf) %>% as.numeric()) %>% 
    na.omit()

    hosps_within <- data.frame(prvnumgrp = df_aha$id, within = st_within(df_aha,sf) %>% as.numeric()) 
...
result %>% 
    bind_rows() %>% 
    filter(within==1) %>% 
    ggplot(aes(x = pct, y = state, colour = geography)) + geom_point() + 
